...
export class HelpDeskAgentPrompts {
  ...
  readonly plannerUserPrompt: string;
  readonly subtaskSystemPrompt: string;
  readonly subtaskToolSelectionUserPrompt: string;
  readonly subtaskReflectionUserPrompt: string;
  readonly subtaskRetryAnswerUserPrompt: string;
  readonly createLastAnswerSystemPrompt: string;
  readonly createLastAnswerUserPrompt: string;
  ...
  constructor(options?: {
    ...
    this.plannerSystemPrompt = options?.plannerSystemPrompt ?? PLANNER_SYSTEM_PROMPT;
    this.plannerUserPrompt = options?.plannerUserPrompt ?? PLANNER_USER_PROMPT;
    this.subtaskSystemPrompt = options?.subtaskSystemPrompt ?? SUBTASK_SYSTEM_PROMPT;
    this.subtaskToolSelectionUserPrompt = options?.subtaskToolSelectionUserPrompt ?? SUBTASK_TOOL_EXECUTION_USER_PROMPT;
    this.subtaskReflectionUserPrompt = options?.subtaskReflectionUserPrompt ?? SUBTASK_REFLECTION_USER_PROMPT;
    this.subtaskRetryAnswerUserPrompt = options?.subtaskRetryAnswerUserPrompt ?? SUBTASK_RETRY_ANSWER_USER_PROMPT;
    this.createLastAnswerSystemPrompt = options?.createLastAnswerSystemPrompt ?? CREATE_LAST_ANSWER_SYSTEM_PROMPT;
    this.createLastAnswerUserPrompt = options?.createLastAnswerUserPrompt ?? CREATE_LAST_ANSWER_USER_PROMPT;
  }

  formatPlannerUserPrompt(vars: { question: string }): string {
    return formatPrompt(this.plannerUserPrompt, vars);
  }

  formatSubtaskToolSelectionUserPrompt(vars: { question: string; plan: string; subtask: string }): string {
    return formatPrompt(this.subtaskToolSelectionUserPrompt, vars);
  }

  formatCreateLastAnswerUserPrompt(vars: { question: string; subtask_results: string }): string {
    return formatPrompt(this.createLastAnswerUserPrompt, vars);
  }
}
